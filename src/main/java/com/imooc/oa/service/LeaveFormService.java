...
public class LeaveFormService {
    ...
    public LeaveForm createLeaveForm(LeaveForm form){
        LeaveForm savedForm = (LeaveForm) MybatisUtils.executeUpdate(sqlSession -> {
            ...
            if(employee.getLevel() < 7){
                ...
                //请假单已提交消息
                String noticeContent = String.format("您的请假申请[%s-%s]已经提交，请等待审批。"
                        , sdf.format(form.getStartTime()), sdf.format(form.getEndTime()));
                noticeDao.insert(new Notice(employee.getEmployeeId(), noticeContent));
                //通知部门经理审批消息
                noticeContent = String.format("%s-%s提起请假申请[%s-%s]，请尽快审批", employee.getTitle(), employee.getName()
                        , sdf.format(form.getStartTime()), sdf.format(form.getEndTime()));
    ...
    public List<Map> getLeaveFormList(String pfState, Long operatorId){
        return (List<Map>) MybatisUtils.executeQuery(sqlSession -> {
            ...
            List<Map> list = dao.selectByParams(pfState, operatorId);
            return list;
        });
    }
    ...
    public void audit(Long formId, Long operatorId, String result, String reason){
        ...
        MybatisUtils.executeUpdate(sqlSession -> {
            ...
            Employee operator = employeeDao.selectById(operatorId);   //表单经办人信息
            NoticeDao noticeDao = sqlSession.getMapper(NoticeDao.class);
